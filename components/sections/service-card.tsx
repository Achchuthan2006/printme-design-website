...
export function ServiceCard({ service, featured = false }: { service: ServiceItem; featured?: boolean }) {
  return (
    <article className="group premium-card flex h-full flex-col rounded-lg border border-line bg-white p-3 shadow-soft hover:border-brand/45 hover:shadow-card">
      <ServiceProductVisual slug={service.slug} />
      <div className="flex flex-1 flex-col px-2 pb-3 pt-4 text-center">
        <div className="mx-auto flex h-9 w-9 items-center justify-center rounded-full border border-brand/20 bg-brand-soft text-brand transition-all duration-300 group-hover:scale-105 group-hover:bg-brand group-hover:text-white">
        ...
        <div className="mt-4">
          <Button href="/quote-request" variant="secondary" className="w-full px-3 py-2 text-xs">
            Quote This
          </Button>
        </div>
      </div>
